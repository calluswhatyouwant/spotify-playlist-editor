...
class PlaylistPage extends Component<Props, State> {
    ...
    handleClick = async () => {
        ...
        sortPlaylistTracksByAttribute(playlistId, playlist.totalTracks, 'name')
            .then(() => getPlaylist(playlistId)
                .then(playlist => this.setState({ playlist, loading: false })));
    };
    ...
    render() {
        ...
        return playlist.name ? (
            <div className="user-playlists-page container my-4 p-4 shadow">
                ...
                <button className="btn btn-dark mr-2" onClick={this.handleClick}>
                    Sort by track name
                </button>
                <PlaylistTrackList loading={loading} tracks={playlist.tracks.items} />
            </div>
        ) : null;
    }
};
